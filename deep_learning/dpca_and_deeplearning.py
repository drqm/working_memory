...
# 更新矩阵形状，使用最小实验数
new_matrix_shape = [min_experiment_count, 306, 2, 2, 501]
...
# 生成第一维的索引，用于后续的训练和测试集分割
experiment_indices = np.arange(min_experiment_count)

# 随机分组，这次包括生成索引的代码
train_indices, test_indices = train_test_split(experiment_indices, test_size=0.3, random_state=42)

# 使用索引分割新矩阵为训练和测试集
train_matrix = new_matrix[train_indices]#(20, 306, 2, 2, 501)
test_matrix = new_matrix[test_indices]

n_components = 5
Z, dpca = dpca_fit(train_matrix, 'bst', n_components)

def fit_and_sort_array(train_matrix):
    new_array = np.zeros([len(train_matrix),7*n_components,2,2,501])
