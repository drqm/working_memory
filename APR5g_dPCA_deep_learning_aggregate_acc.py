...
periods = ['baseline','listen','imagine']
...
for s in subjects:
    cacc = []
    for p in periods:
        f = acc_dir + s + '_accuracy_' + p + '_False_2.p'
        with open(f,'rb') as cf:
            cacc += [pickle.load(cf)]
    all_data += [cacc]
    
all_data = np.mean(np.array(all_data)[...,11:21],axis=(2,3))

n = all_data.shape[0]

## Plot accuracies
fig, ax = plt.subplots()
ax.boxplot(all_data*100, widths = 0.05,showfliers=False,positions=[1.075,2.075,3.075])
