...
#save output:
if save_averages:
    evkd_fname = op.join(avg_path,'data',sub + '_evoked_smooth.p')
...
plt.tight_layout()
plt.savefig(avg_path + '/figures/{}_ERFs_topo_mag_smooth.pdf'.format(sub))
...
plt.tight_layout()
plt.savefig(avg_path + '/figures/{}_ERFs_topo_grad_smooth.pdf'.format(sub))
...
plt.savefig(avg_path + '/figures/{}_ERFs_diff_topo_grad_smooth.pdf'.format(sub))

fig4, axis = plt.subplots(nrows=1,ncols=1,figsize=(60,30))
mne.viz.plot_evoked_topo(diff.copy().pick_types('mag'),
                         axes = axis)
plt.tight_layout()
plt.savefig(avg_path + '/figures/{}_ERFs_diff_topo_mag_smooth.pdf'.format(sub))
...
plt.tight_layout()
plt.savefig(avg_path + '/figures/{}_ERFs_topo_mag_mels_maint_smooth.pdf'.format(sub))
...
mne.viz.plot_evoked_topo([evokeds['maint/mel1'].copy().pick_types('grad'),
                         ...
                         axes = axis,merge_grads=True)
plt.tight_layout()
plt.savefig(avg_path + '/figures/{}_ERFs_topo_grad_mels_maint_smooth.pdf'.format(sub))

fig5, axis = plt.subplots(nrows=1,ncols=1,figsize=(60,30))
mne.viz.plot_evoked_topo([evokeds['manip/mel1'].copy().pick_types('mag'),
                          evokeds['manip/mel2'].copy().pick_types('mag')],axes = axis)
plt.tight_layout()
plt.savefig(avg_path + '/figures/{}_ERFs_topo_mag_mels_manip_smooth.pdf'.format(sub))
...
#                                        tmin= 0, tmax = 6.25,rank ='info')
    ...
    for e in evokeds:
        ...
        #                                                 lambda2=1/SNR**2)
        sources[e] = mne.beamformer.apply_lcmv_epochs(evokeds[e],inv)#,max_ori_out='signed')
    ...
    src_fname = op.join(avg_path,'data',sub + '_evoked_sources_smooth.p')
    src_file = open(src_fname,'wb')
    pickle.dump(sources,src_file)
    src_file.close()

    inv_fname = op.join(avg_path,'data',sub + '_evoked_inverse_smooth.p')
