...
events = events[events[:,2] < 4] # delete spurious events >
events_orig = events.copy()
#events2 = events.copy()
events[:,0] = events[:,0] + 400
...
epochs_orig = mne.Epochs(raw, events = events_orig, event_id = event_id,
                tmin = tmin, tmax = tmax, picks = picks, 
...
# # scores = time_gen.score(X=epochs_orig.get_data(),
# #                          y=epochs_orig.events[:, 2])

# time_gen.fit(X=epochs_orig.get_data(),
#               y=epochs_orig.events[:, 2])
...
for b in [blocks[1]]:
    ...
    # events = events[events[:,2] < 100,:]
    #events2[:,2] = events2[:,2]%10 # recode events
    events2[:,2] = (events2[:,2]%10 - 4)*-1
    events2[:,0] = events2[:,0] + 400
    #Epoching and averaging:
    event_id = {'1': 1, '3': 3}
    picks = mne.pick_types(raw.info, meg = True, eog = False)
    tmin, tmax = -1.5,4 #epoch time
    baseline = (-0.1,0) # baseline time
    reject = dict(mag = 4e-12, grad = 4000e-13)#, eog = 250e-6)
    
    #epoching:
    epochs2 = mne.Epochs(raw, events = events2, event_id = event_id,
                    tmin = tmin, tmax = tmax, picks = picks, 
                    baseline = baseline,  decim = 2) #reject = reject,
...
#    pred = time_gen.predict_proba(epochs2[52].get_data())
    ...
    #plotting                                                                
    fig, ax = plt.subplots(1)
    im = ax.matshow(scores, vmin=0.2, vmax=0.8, cmap='RdBu_r', origin='lower',
                    extent = [-1.5,4,-0.1,1])#epochs.times[[0, -1, 0, -1]])
    ax.axhline(0., color='k')
    ax.axvline(0., color='k')
    ax.xaxis.set_ticks_position('bottom')
    ax.set_xlabel('Testing Time (s)')
...
# coef = get_coef(time_gen,'filters_',inverse_transform = True)
